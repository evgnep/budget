...
import su.nepom.budget.db.sqlite.mapping.toItemAndNo
import su.nepom.budget.db.sqlite.mapping.toTransaction
import su.nepom.budget.db.sqlite.mapping.toTransactions
import su.nepom.budget.db.sqlite.mapping.toTransactionsMap
...
internal class SqliteTransactionDao(override val session: SqliteSession) : TransactionDao, DatabaseHolder {
  ...
  override fun getItemsByQuery(query: TransactionDao.Query): List<TransactionContextItemAndTransaction> =
    session.doReadOp {
      data class ItemInfo(val item: TransactionContentItem, val no: Int, val transactionUuid: String)

      val itemsWithInfo: List<ItemInfo> = fromTransactions()
        .select(
          TransactionItems.transactionUuid,
          TransactionItems.accountUuid,
          TransactionItems.money,
          TransactionItems.description,
          TransactionItems.flag,
        ...
        .map { rs ->
          ...
          ItemInfo(item, no, rs.getString(1)!!)
        }
      ...
      val map: Map<String, Pair<TransactionContent, MutableList<Pair<TransactionContentItem, Int>>>> =
        ...
        .orderBy(query)
        .toTransactionsMap()
      itemsWithInfo.mapIndexed { index, it ->
        val transactionInfo = requireNotNull(map[it.transactionUuid]) { "Transaction ${it.transactionUuid} not found" }
        val isFirstInGroup = index == 0 || itemsWithInfo[index - 1].transactionUuid != it.transactionUuid
        TransactionContextItemAndTransaction(it.item, it.no, isFirstInGroup, transactionInfo.toTransaction())
      }
    }

  override fun countItemsByFilter(filter: TransactionDao.Filter): Int = session.doReadOp {
    fromTransactions()
      .select(count(TransactionItems.id))
      .where(filter)
      .map { it.getInt(1) }.single()
  }
  ...
  private fun Query.where(filter: TransactionDao.Filter) =
    whereWithConditions { conditions ->
      ...
      filter.amount?.let { amount ->
        amount.min?.let { conditions.add(TransactionItems.money gte it.value) }
        amount.max?.let { conditions.add(TransactionItems.money lte it.value) }
      }
